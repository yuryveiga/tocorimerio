...
export function Header({ forceLanguage }: { forceLanguage?: 'pt' | 'en' | 'es' }) {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 pointer-events-none">
      ...
      {/* Quick Contact Bar — hidden on scroll to keep header compact */}
      {!isScrolled && (
        <div className="hidden md:flex items-center justify-end gap-6 px-6 py-1.5 text-[11px] font-semibold text-muted-foreground border-b border-border/30 bg-background/60">
          {(() => {
            ...
            const displayNumber = `+${waNumber.replace(/(\d{2})(\d{2})(\d{5})(\d{4})/, '$1 ($2) $3-$4')}`;
            return (
              <>
                <a
                  href={waLink}
                  ...
                  className="inline-flex items-center gap-1.5 text-[#25D366] hover:text-[#1aad54] transition-colors font-bold"
                >
                  ...
                  {language === 'pt' ? 'WhatsApp — Respondemos em minutos' : language === 'es' ? 'WhatsApp — Respondemos en minutos' : 'WhatsApp — We reply in minutes'}
                </a>
                <span className="opacity-30">|</span>
                <a href={`tel:${displayNumber}`} className="hover:text-foreground transition-colors">
                  📞 {displayNumber}
                </a>
              </>
            );
