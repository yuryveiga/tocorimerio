    ...
    <div className="fixed right-4 bottom-4 md:right-6 md:bottom-6 z-50 flex flex-col gap-2 md:gap-3 items-end pointer-events-auto">
      ...
      {whatsapp && (
        <button
          ...
          className="relative flex items-center gap-3 bg-[#25D366] text-white pl-4 pr-5 h-14 rounded-full shadow-[0_4px_30px_rgba(37,211,102,0.55)] hover:shadow-[0_6px_40px_rgba(37,211,102,0.75)] hover:scale-105 active:scale-95 transition-all group"
          aria-label="WhatsApp"
        >
          {/* Pulse ring */}
          ...
          <svg viewBox="0 0 24 24" className="w-6 h-6 shrink-0 relative z-10" fill="currentColor">
          ...
          <span className="relative z-10 text-xs font-black uppercase tracking-wider leading-tight text-left">
            {language === 'pt' ? (
              <><span className="block">Fale Agora</span><span className="block opacity-80 font-medium normal-case tracking-normal">Resposta imediata</span></>
            ) : language === 'es' ? (
              <><span className="block">Habla Ahora</span><span className="block opacity-80 font-medium normal-case tracking-normal">Respuesta inmediata</span></>
            ) : (
              <><span className="block">Chat Now</span><span className="block opacity-80 font-medium normal-case tracking-normal">Instant reply</span></>
            )}
          </span>
        </button>
      )}
